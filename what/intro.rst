...
致云云管理平台是为了提供高效、简洁的云计算服务而设计的纯软件IaaS平台。

平台使用类似P2P模式的分布式架构，平台各组件能够自动识别、自动组网、自动管理，无需人工干预。用户可以从任意地点，通过纯图形化的Web界面对平台进行管理和监控，极大程度降低维护成本和学习难度。创建完成的云主机有独立的数据链路进行访问，与管理数据流分离，即使在平台管理组件故障的情况下，依然不会影响云主机的运行和服务。

.. image:: images/01_component_overview.png



各组件之间使用专用协议对等通讯，各组件均为纯软件模块，可以在任意标准x86服务器上灵活安装，既能将多组件集中部署到一台服务器作为All In One服务，也可以部署多个同类型节点服务器，组成集群，提供高可用及负载分担，满足各种规模和不同场景下的云计算平台部署需要
